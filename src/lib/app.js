        ...
        async function authRpcCall(rpcName, params, timeoutMs = SESSION_CHECK_TIMEOUT) {
            ...
            try {
                ...
                if (error) {
                    // 判断是否为网络/超时类错误
                    if (error?.code === 'AbortError' || error?.message?.includes('timeout') || error?.message?.includes('网络')) {
        ...
        function updateLoginStatus() {
            ...
            if (goUserCenterBtn) {
                goUserCenterBtn.innerHTML = isLoggedIn
                    ? '<i class="fas fa-user-gear"></i> 用户中心'
                    : '<i class="fas fa-user-gear"></i> 前往用户中心';
            }
            if (profileAvatar) {
        ...
        async function autoLogin() {
            ...
            try {
                await initSupabase();
                if (!supabaseClient) {
                    console.warn('Supabase 未初始化，跳过自动登录验证');
                    updateLoginStatus();
                    return;
                }
        ...
        async function updateSiteVisitCount() {
            ...
            try {
                initSupabase();
                if (!supabaseClient) {
                    console.warn('Supabase 未初始化，跳过统计');
                    if (visitCountElement) {
                    ...
                    showToast('获取访问次数失败：Supabase 未初始化', 'error');
                    return;
                }
                // 查询当前访问次数
                const { data, error } = await supabaseClient
            ...
            onError(e) {
                ...
                if (currentSong.platform === 'bili') {
                    if (this.retryCount >= this.maxRetries) {
                        showToast('Bilibil音频加载失败，请检查网络或重新搜索', 'error');
                        this.retryCount = 0;
                        return;
                    }
                    this.retryCount++;
                ...
                // 已达最大重试次数，停止重试
                if (this.retryCount >= this.maxRetries) {
                    showToast('多次播放失败，请检查网络或稍后再试', 'error');
                    this.retryCount = 0;
                    return;
                }
        ...
        async function makeApiRequest(url, params, useCache = true) {
            ...
            try {
                ...
                if (isBiliApi) {
                    ...
                    throw new Error(`Bilibili API调用失败: ${error.message}\n可能是CORS限制或网络问题，请稍后再试`);
                }
                throw error;
            }
        }
        ...
        async function playSong(songId, platform = null, songInfo = null) {
            try {
                ...
                if ((result.code === 1 || result.code === 200) && result.data && result.data.url) {
                    ...
                    showToast('无法获取播放链接，请检查网络或Token', 'error');
                }
            } catch (error) {
